...
def plot_fft(data, output_image):
    ...
    for i, (x_data, y_data) in enumerate(data):
        ...
        plt.plot(freqs, np.abs(y_fft), label=f"FFT of Data {i+1} - Temp: 285.15K")
...
def plot_ifft(data, output_image):
    ...
    for i, (x_data, y_data) in enumerate(data):
        ...
        plt.plot(x_data_shifted, np.real(y_ifft), label=f"IFFT of Data {i+1} - Temp: 285.15K")
...
def plot_filtered_ifft(data, output_image, threshold=0.1):
    ...
    for i, (x_data, y_data) in enumerate(data):
        ...
        plt.plot(x_data_shifted, np.real(y_ifft_filtered), label=f"FilterData {i+1} -Temp:285.15K")
...
def process_files(files=None, initial_guess=None, output_image='fit_plot.png'):
    ...
    for i, (x_data, y_data) in enumerate(data):
        ...
        plt.plot(
            x_data_shifted, y_data_shifted, label=f'Original Data {i + 1} - Temp: 285.15K'
        )
