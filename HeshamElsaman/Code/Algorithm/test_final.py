...
def test_list_files(tmp_path):
    """
    Test the list_files function with a temporary directory.
    """
    ...
    result = final.list_files(tmp_path, extension, "experiment")
    ...
    result = final.list_files(tmp_path, extension, "nonexistent")
    expected = []
    assert result == expected, f"Test failed: Expected {expected}, but got {result}."
    # Test case 3: Empty directory
    empty_dir = tmp_path / "empty"
    empty_dir.mkdir()
    result = final.list_files(empty_dir, extension, "experiment")
    expected = []
    assert result == expected, f"Test failed: Expected {expected}, but got {result}."
    ...
    result = final.list_files(directory, extension, "gps_sin")
    assert result == expected, f"Test failed: Expected {expected}, but got {result}."

# Testing Listing the Folders
def test_list_folders(tmp_path):
    """
    Test the list_folders function with a temporary directory.
    """
