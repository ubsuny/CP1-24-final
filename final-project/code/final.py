...
def parse_temperature_from_markdown(file_path):
    ...
    try:
        ...
        # Look for temperature information in the file
        for line in lines:
            if "Temperature" in line and "°F" in line:
                # Split line into parts based on delimiter
                parts = line.split('|')
                if len(parts) > 2:
...
def non_linear_fitting(params):
    ...
    for _ in range(max_iterations):
        ...
        # Compute gradients using helper function
        gradients = _compute_gradients(fit_function, params, x_sampled, y_pred, residuals)
...
def _compute_gradients(fit_function, params, x_sampled, y_pred, residuals):
    """
    Helper function to compute gradients for parameter updates.
    """
    gradients = []
    for i in range(len(params)):
        delta = np.zeros_like(params)
        delta[i] = 1e-3
        y_pred_step = fit_function(x_sampled, *(params + delta))
        gradient = np.sum(2 * residuals * (y_pred_step - y_pred) / delta[i])
